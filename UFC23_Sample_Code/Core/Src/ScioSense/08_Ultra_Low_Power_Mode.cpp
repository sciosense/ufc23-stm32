...
static void UFC23_EnterUltraLowPowerMode();
...
extern "C" void UFC23_Example_Setup(UART_HandleTypeDef *uart, SPI_HandleTypeDef *spi)
{
    ...
    if( ufc23.isMeasuring() )
    {
        // Wake from shutdown. No need to re-configure
    ...
    else
    {
        ...
        SerialPrint("\nStarting UFC23 08_Ultra_Low_Power_Mode demo on STM32...\n");
...
extern "C" void UFC23_Example_Loop()
{
    while( 1 )
    {
        if( interruptAsserted )
        {
            ...
            {
                // The high speed clock calibration must be calculated first to get the conversion to ToF in nanoseconds
                if( ufc23.getHighSpeedClockFrequencyHz() )
                {
                    ...
                    {
                        sprintf(messageBuffer, "Bundle:%02d\t", bundleIdx);
                        SerialPrint(messageBuffer);
...
static void UFC23_EnterUltraLowPowerMode()
{
    HAL_PWR_EnableWakeUpLine(PWR_WAKEUP_LINE6, PWR_WAKEUP_SELECT_0, PWR_WAKEUP_POLARITY_LOW);
    // Place into shutdown mode
